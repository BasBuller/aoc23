function check_adjacent_symbol(number::RegexMatch, width::Int, text::String, pattern::Regex)
    above_start = max(1, number.offset - width - 1)
    above_end = max(1, above_start + length(number.match) + 1)
    left_idx = number.offset - 1
    right_idx = number.offset + length(number.match)
    below_start = min(length(text), number.offset + width - 1)
    below_end = min(length(text), below_start + length(number.match) + 1)

    surrounding_text = text[above_start:above_end] * text[left_idx] * text[right_idx] * text[below_start:below_end]
    has_symbol = match(pattern, surrounding_text)
    return !isnothing(has_symbol), has_symbol
end


function engine1(input::String)
    width = split(input, "\n") |> first |> length
    width += 2
    input = ("."^(width + 1)) * replace(input, "\n"=>"..") * ("."^(width + 1))
    
    total = 0
    for number in eachmatch(r"[0-9]+", input)
        if first(check_adjacent_symbol(number, width, input, r"(?!([0-9]+|\.+))."))
            total += parse(Int, number.match)
        end
    end
...
function pad_input(input::String)
    width = match(r"\n", input).offset - 1
    input = "."^(width + 1) * "\n" * input * "\n" * "."^(width + 1)
    input = replace(input, "\n"=>".\n.")
    return input
...
function insert_or_append!(collection::Dict, key, value)
    values = get(collection, key, [])
    push!(values, value)
...
function make_number_index(input::String)
    indices = Dict()
    for (row_idx, row) in enumerate(split(input, '\n'))
        for number in eachmatch(r"[0-9]+", row)
            num = parse(Int, number.match)
            for offset in -1:length(number.match)
                idx_above = (row_idx - 1, number.offset + offset)
                insert_or_append!(indices, idx_above, num)
                idx_below = (row_idx + 1, number.offset + offset)
                insert_or_append!(indices, idx_below, num)
            end
            idx_left = (row_idx, number.offset - 1)
            insert_or_append!(indices, idx_left, num)
            idx_right = (row_idx, number.offset + length(number.match))
            insert_or_append!(indices, idx_right, num)
        end
    end
    return indices
end
...
function gears(input::String)
    padded_input = pad_input(input)
    pair_index = Dict()
    for (key, value) in make_number_index(padded_input)
        if length(value) == 2
            pair_index[key] = value
        end
    end

    total = 0
    for (row_idx, row) in enumerate(split(padded_input, '\n'))
        for gear in eachmatch(r"\*", row)
            key = (row_idx, gear.offset)
            if haskey(pair_index, key)
                total += prod(pair_index[key])
            end
        end
    end
...
...\$.*....
.664.598.."
@show engine1(test_input)
@show gears(test_input)
