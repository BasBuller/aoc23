...
function linevalue(line)
    iter = eachmatch(r"([0-9]|one|two|three|four|five|six|seven|eight|nine)", line; overlap=true)
    first_val = first(iter)
    second_val = nothing
    for val in iter 
        second_val = val
    end
    return parse_match(first_val) * 10 + parse_match(second_val)
end
function day2(text::String)
    accum = 0
    for line in split(text, "\n")
        accum += linevalue(line)
    end
    return accum
end

inputs = "99lbqpxzzlbtvkmfrvrnmcxttseven
q7cnfslbtpkvseven
