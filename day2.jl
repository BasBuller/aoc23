function game_max(text)
    red_max, green_max, blue_max = 0, 0, 0
    for draws in split(text, "; ")
        for draw in split(draws, ", ")
            count, color = split(draw, " ")  
            count = parse(Int, count)
            if (color == "red") & (count > red_max)
                red_max = count
            elseif (color == "green") & (count > green_max)
                green_max = count
            elseif (color == "blue") & (count > blue_max)
                blue_max = count
            end
        end
    end
...
function part1(text, red_max, green_max, blue_max)
    possible_game_ids = []
    games = split(text, "\n")
    for (idx, game) in enumerate(games)
        draws = split(game, ": ")[2]
        red_draw, green_draw, blue_draw = game_max(draws)
        if (red_draw <= red_max) & (green_draw <= green_max) & (blue_draw <= blue_max)
            append!(possible_game_ids, idx)
        end
    end
...
function part2(text)
    ...
    for game in games
        draws = split(game, ": ")[2]
        max_draws = game_max(draws)
        append!(powers, reduce(*, max_draws))
    end
    return sum(powers)
end
...
inputs = "Game 1: 3 blue, 2 green, 6 red; 17 green, 4 red, 8 blue; 2 red, 1 green, 10 blue; 1 blue, 5 green
...
@show part1(test, 12, 13, 14)
@show part1(inputs, 12, 13, 14)
println()
@show part2(test)
@show part2(inputs)
